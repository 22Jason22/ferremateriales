...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const userToolsDiv = document.getElementById('user-tools');
    if (userToolsDiv) {
        // Remove default Django admin elements from user-tools
        const defaultWelcomeStrong = userToolsDiv.querySelector('strong');
        if (defaultWelcomeStrong) {
            // Remove the "Welcome," text node if it exists before the strong tag
            let previousSibling = defaultWelcomeStrong.previousSibling;
            while (previousSibling && previousSibling.nodeType === Node.TEXT_NODE && previousSibling.textContent.trim() === 'Welcome,') {
                userToolsDiv.removeChild(previousSibling);
                previousSibling = defaultWelcomeStrong.previousSibling;
            }
            userToolsDiv.removeChild(defaultWelcomeStrong);
        }

        const defaultLinks = userToolsDiv.querySelectorAll('a[href="/"], a[href*="/admin/doc/"], a[href*="/admin/password_change/"]');
        defaultLinks.forEach(link => {
            // Remove the link and any trailing '/' text node
            let nextSibling = link.nextSibling;
            if (nextSibling && nextSibling.nodeType === Node.TEXT_NODE && nextSibling.textContent.trim() === '/') {
                userToolsDiv.removeChild(nextSibling);
            }
            userToolsDiv.removeChild(link);
        });

        const defaultLogoutForm = userToolsDiv.querySelector('#logout-form');
        if (defaultLogoutForm) {
            userToolsDiv.removeChild(defaultLogoutForm);
        }

        // Create a container for the new custom elements
        const customElementsContainer = document.createElement('div');
        customElementsContainer.className = 'custom-navbar-elements';
        customElementsContainer.style.display = 'flex';
        customElementsContainer.style.alignItems = 'center';

        // Append new elements in order
        customElementsContainer.appendChild(createAppsDropdown());
        customElementsContainer.appendChild(createUserDropdown());
        customElementsContainer.appendChild(createLanguageSelector());

        // Find the existing theme toggle and append it
        const existingThemeToggle = userToolsDiv.querySelector('.theme-toggle');
        if (existingThemeToggle) {
            customElementsContainer.appendChild(existingThemeToggle);
        }

        // Prepend the custom elements container to userToolsDiv
        userToolsDiv.prepend(customElementsContainer);
    }
